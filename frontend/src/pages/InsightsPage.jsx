...
export default function InsightsPage() {
  ...
  const maxWear = Math.max(...(stats.most_worn?.map(i => parseInt(i.wear_count)) || [1]), 1);
      ...
      <div className="content">
        <div className="stats-grid">
          <div className="stat-card">
            ...
            <div className="stat-val">{stats.total_items}</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">Saved outfits</div>
            <div className="stat-val">{stats.total_outfits}</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">Logged this month</div>
            <div className="stat-val">{stats.logs_this_month}</div>
            <div className="stat-sub">outfits</div>
          </div>
          <div className="stat-card">
            <div className="stat-label">Never worn</div>
            <div className="stat-val" style={{ color: stats.never_worn?.length > 0 ? 'var(--red-dark)' : 'inherit' }}>
          ...
          <div className="card" style={{ padding:16, marginBottom:16 }}>
            ...
            {stats.most_worn.map(item => (
              <div key={item.id} className="bar-row">
                <span style={{ fontSize:18 }}>{CATEGORY_EMOJI[item.category] || '👚'}</span>
                <span className="bar-label">{item.name}</span>
                <div className="bar-track">
                  <div className="bar-fill" style={{ width: `${Math.round(parseInt(item.wear_count) / maxWear * 100)}%` }} />
                </div>
        ...
        {stats.wear_by_month?.length > 0 && (
          <div className="card" style={{ padding:16, marginBottom:16 }}>
            ...
            {(() => {
              const maxCount = Math.max(...stats.wear_by_month.map(m => parseInt(m.count)), 1);
              return stats.wear_by_month.map(m => (
                <div key={m.month} className="bar-row">
                  ...
                  <div className="bar-track">
                    <div className="bar-fill" style={{ width:`${Math.round(parseInt(m.count)/maxCount*100)}%` }} />
                  </div>
        ...
        {stats.never_worn?.length > 0 && (
          <div style={{ background:'var(--red-light)', border:'0.5px solid var(--red-mid)', borderRadius:'var(--radius)', padding:16 }}>
            <div style={{ fontSize:13, fontWeight:600, color:'var(--red-dark)', marginBottom:8 }}>
              ⚠️ {stats.never_worn.length} items never worn
            </div>
        ...
        {stats.category_breakdown?.length > 0 && (
          <div className="card" style={{ padding:16, marginTop:16 }}>
            <div style={{ fontSize:13, fontWeight:600, marginBottom:14 }}>Closet breakdown</div>
            {stats.category_breakdown.map(c => (
              <div key={c.category} className="bar-row">
                ...
                <div className="bar-track">
                  <div className="bar-fill" style={{ width:`${Math.round(parseInt(c.count)/stats.total_items*100)}%` }} />
                </div>
