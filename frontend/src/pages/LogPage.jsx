...
function LogModal({ onClose, onSave, date }) {
  ...
  const [mode, setMode] = useState('outfit'); // 'outfit' | 'custom'
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        <div className="modal-title">Log outfit for {new Date(date + 'T12:00:00').toLocaleDateString('en-US', { weekday:'long', month:'long', day:'numeric' })}</div>
        <div className="filter-row" style={{ marginBottom:16 }}>
          <button className={`chip${mode==='outfit'?' active':''}`} onClick={() => setMode('outfit')}>Saved outfit</button>
          <button className={`chip${mode==='custom'?' active':''}`} onClick={() => setMode('custom')}>Pick individual items</button>
        </div>
        {mode === 'outfit' ? (
          <div style={{ display:'flex', flexDirection:'column', gap:8, marginBottom:16 }}>
            {outfits.map(o => (
              <div key={o.id} onClick={() => setSelectedOutfit(o.id)}
                style={{ display:'flex', alignItems:'center', gap:12, padding:'10px 12px', border:`1.5px solid ${selectedOutfit===o.id?'var(--purple)':'var(--border)'}`,
                  borderRadius:8, cursor:'pointer', background:selectedOutfit===o.id?'var(--purple-light)':'var(--bg-card)' }}>
                <span style={{ fontSize:20 }}>
                  {o.items?.slice(0,2).map(i => CATEGORY_EMOJI[i.category] || '👚').join('')}
                </span>
                <div>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(80px, 1fr))', gap:8, marginBottom:16 }}>
            {allItems.map(item => (
              <div key={item.id} onClick={() => toggleItem(item.id)}
                style={{ border:`1.5px solid ${selectedItems.has(item.id)?'var(--purple)':'var(--border)'}`,
                  borderRadius:8, overflow:'hidden', cursor:'pointer', background:selectedItems.has(item.id)?'var(--purple-light)':'var(--bg-card)' }}>
                {item.photo_url
        ...
        <div className="form-group">
          ...
          <input className="form-input" value={notes} onChange={e => setNotes(e.target.value)} placeholder="How did it feel? Any tweaks?" />
        </div>
    ...
    <>
      ...
      {!todayLog && (
        <div className="banner banner-teal">
          ...
          <div className="banner-action" onClick={() => setShowLog(true)}>Log now →</div>
        </div>
      )}
      ...
      <div className="content">
        {logs.length === 0 ? (
          <div style={{ textAlign:'center', padding:'40px', color:'var(--text-3)' }}>
            No logs yet. Start by logging today's outfit!
          </div>
        ) : (
          logs.map(log => {
            ...
            return (
              <div key={log.id} className="log-row">
                ...
                <div style={{ display:'flex', gap:6, fontSize:22 }}>
                  {log.items?.slice(0,4).map(i => <span key={i.id}>{CATEGORY_EMOJI[i.category] || '👚'}</span>)}
      ...
      {showLog && <LogModal date={today} onClose={() => setShowLog(false)} onSave={() => { setShowLog(false); load(); }} />}
    </>
  );
}
