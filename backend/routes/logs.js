...
// GET logs (paginated, most recent first)
router.get('/', async (req, res) => {
  const db = req.app.locals.db;
  try {
    const { limit = 30, offset = 0 } = req.query;
    const logs = await db.query(`
      SELECT ol.*, o.name as outfit_name
      FROM outfit_logs ol
...
// POST log an outfit worn today (or any date)
router.post('/', async (req, res) => {
  ...
  try {
    ...
    // If logging individual items without a saved outfit, create a temporary one
