...
router.get('/', async (req, res) => {
  ...
  try {
    const outfits = await db.query(`
      SELECT o.*, COUNT(DISTINCT ol.id)::int as wear_count, MAX(ol.worn_date) as last_worn
      FROM outfits o
      LEFT JOIN outfit_logs ol ON ol.outfit_id = o.id
      GROUP BY o.id ORDER BY o.created_at DESC
    `);
    const items = await db.query(`
      SELECT oi.outfit_id, i.* FROM outfit_items oi JOIN items i ON i.id = oi.item_id
    `);
    const byOutfit = {};
    items.rows.forEach(item => {
      if (!byOutfit[item.outfit_id]) byOutfit[item.outfit_id] = [];
      byOutfit[item.outfit_id].push(item);
    });
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const outfit = await db.query(
      'INSERT INTO outfits (name, notes) VALUES ($1,$2) RETURNING *', [name, notes]
    );
    const id = outfit.rows[0].id;
    if (item_ids?.length) {
      await Promise.all(item_ids.map(iid =>
        db.query('INSERT INTO outfit_items (outfit_id, item_id) VALUES ($1,$2)', [id, iid])
      ));
    }
    const items = await db.query(
      'SELECT i.* FROM items i JOIN outfit_items oi ON oi.item_id=i.id WHERE oi.outfit_id=$1', [id]
    );
...
router.patch('/:id', async (req, res) => {
  ...
  try {
    ...
    if (item_ids) {
      ...
      await Promise.all(item_ids.map(iid =>
        db.query('INSERT INTO outfit_items (outfit_id, item_id) VALUES ($1,$2)', [req.params.id, iid])
      ));
    }
    const outfit = await db.query('SELECT * FROM outfits WHERE id=$1', [req.params.id]);
    const items = await db.query(
      'SELECT i.* FROM items i JOIN outfit_items oi ON oi.item_id=i.id WHERE oi.outfit_id=$1', [req.params.id]
    );
    res.json({ ...outfit.rows[0], items: items.rows });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
